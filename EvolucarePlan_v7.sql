...
---------             Compatible SQL Server 2005, SQL Server 2008, -------------------
--------- SQL Server 2008 R2, SQL Server 2012, SQL Server 2014 and -------------------
---------							SQL Server 2016                -------------------
--------------------------------------------------------------------------------------
...
CREATE DATABASE [Evolucare_MaintenancePlan]
 GO

ALTER DATABASE [Evolucare_MaintenancePlan] MODIFY FILE (NAME = Evolucare_MaintenancePlan, FILEGROWTH = 8MB, SIZE = 32MB, MAXSIZE = UNLIMITED);  

 
GO  
ALTER DATABASE [Evolucare_MaintenancePlan] MODIFY FILE (NAME = Evolucare_MaintenancePlan_log, FILEGROWTH = 8MB, SIZE = 32MB, MAXSIZE = UNLIMITED); 
 
 GO
 ALTER DATABASE [Evolucare_MaintenancePlan] SET RECOVERY SIMPLE 
GO

USE [Evolucare_MaintenancePlan] -- Specify the database in which the objects will be created.
GO
...
SET @BackupDirectory     = NULL         -- Specify the backup root directory. Example SET @BackupDirectory = N'D:\Sauvegardes_SQL'
SET @CleanupTime         = 96         -- Time in hours, after which backup files are deleted. If no time is specified, then no backup files are deleted. (1=24, 2=48, 3=72, 4=96, 5=120, 6=144, 7=168, etc.)
...
BEGIN
  ...
  SET @JobDescription = 'Plan de Maintenance EVOLUCARE v7'
  SET @JobCategory = 'Database Maintenance'
  SET @JobOwner = SUSER_SNAME(0x01)

  SET @JobName01 = 'EvolucarePlan v7 | Backup - ALL_DB - FULL'
  SET @JobCommand01 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[DatabaseBackup] @Databases = ''ALL_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''FULL'', @Verify = ''Y'', @CleanupTime = ' + ISNULL(CAST(@CleanupTime AS nvarchar),'NULL') + ', @CheckSum = ''Y''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  ...
  SET @JobName02 = 'EvolucarePlan v7 | Backup - USER_DB - DIFF'
...
SET @JobName04 = 'EvolucarePlan v7 | Backup - USER_DB - LOG'
  ...
  SET @JobName03 = 'EvolucarePlan v7 | IndexOptimize - Stats Only'
  ...
  SET @JobName05 = 'EvolucarePlan v7 | IntegrityCheck - ALL_DB'
  SET @JobCommand05 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[DatabaseIntegrityCheck] @Databases = ''ALL_DATABASES''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  ...
  SET @JobName06 = 'EvolucarePlan v7 | IndexOptimize - Semaine 30-50'
  SET @JobCommand06 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [dbo].[IndexOptimize] @Databases = ''USER_DATABASES''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  SET @OutputFile06 = COALESCE(@OutputFileDirectory,@TokenLogDirectory) + '\' + 'IndexOptimize_' + @TokenJobID + '_' + @TokenStepID + '_' + @TokenDate + '_' + @TokenTime + '.txt'
  ...
  SET @JobName07 = 'EvolucarePlan v7 | IndexOptimize - Weekend 10-30'
  ...
  SET @JobName08 = 'EvolucarePlan v7 | SP_delete_backuphistory'
  ...
  SET @JobName09 = 'EvolucarePlan v7 | SP_purge_jobhistory'
  ...
  SET @JobName10 = 'EvolucarePlan v7 | Output File Cleanup'
  ...
  SET @JobName11 = 'EvolucarePlan v7 | CommandLog Cleanup'
  ...
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName01, @step_name = @JobName01, @subsystem = 'CMDEXEC', @command = @JobCommand01, @retry_attempts=3, @retry_interval=1, @output_file_name = @OutputFile01
  ...
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName02, @step_name = @JobName02, @subsystem = 'CMDEXEC', @command = @JobCommand02, @retry_attempts=3, @retry_interval=1, @output_file_name = @OutputFile02
  ...
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName03, @step_name = @JobName03, @subsystem = 'CMDEXEC', @command = @JobCommand03, @retry_attempts=3, @retry_interval=1, @output_file_name = @OutputFile03
  ...
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName04, @step_name = @JobName04, @subsystem = 'CMDEXEC', @command = @JobCommand04, @retry_attempts=3, @retry_interval=1, @output_file_name = @OutputFile04
  ...
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName05, @step_name = @JobName05, @subsystem = 'CMDEXEC', @command = @JobCommand05, @retry_attempts=3, @retry_interval=1, @output_file_name = @OutputFile05
  ...
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName06, @step_name = @JobName06, @subsystem = 'CMDEXEC', @command = @JobCommand06, @retry_attempts=3, @retry_interval=1, @output_file_name = @OutputFile06
  ...
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName07, @step_name = @JobName07, @subsystem = 'CMDEXEC', @command = @JobCommand07, @retry_attempts=3, @retry_interval=1, @output_file_name = @OutputFile07
  ...
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName08, @step_name = @JobName08, @subsystem = 'CMDEXEC', @command = @JobCommand08, @retry_attempts=3, @retry_interval=1, @output_file_name = @OutputFile08
  ...
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName09, @step_name = @JobName09, @subsystem = 'CMDEXEC', @command = @JobCommand09, @retry_attempts=3, @retry_interval=1, @output_file_name = @OutputFile09
  ...
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName10, @step_name = @JobName10, @subsystem = 'CMDEXEC', @command = @JobCommand10, @retry_attempts=3, @retry_interval=1, @output_file_name = @OutputFile10
  ...
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_jobstep @job_name = @JobName11, @step_name = @JobName11, @subsystem = 'CMDEXEC', @command = @JobCommand11, @retry_attempts=3, @retry_interval=1, @output_file_name = @OutputFile11

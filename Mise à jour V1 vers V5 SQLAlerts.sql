	...
	SET @procSQL = N'
		...
		WHERE schedule_id = ( 
			SELECT schedule_id FROM dbo.sysjobschedules WHERE job_id = ( 
				SELECT job_id
				FROM dbo.sysjobs
				WHERE dbo.sysjobs.Name = ''Plan de maintenance Evolucare (vTSQL).Sauvegarde des BDD''
			)
		)'
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Plan de maintenance Evolucare (v4TSQL).Sauvegarde des BDD', 
		@enabled=1, 
